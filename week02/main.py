...
for step in range(15 ** model.nq):
    indices = np.unravel_index(step, (15,) * model.nq)
    configuration = [np.linspace(model.jnt_range[j][0], model.jnt_range[j][1], 15)[indices[j]] for j in
                         range(model.nq)]
